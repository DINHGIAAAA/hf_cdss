...
import { ChevronRight, LoaderCircle, Pill, RefreshCw } from "lucide-react";
...
export function DoseRulesPage() {
  ...
  const loadRules = useCallback(async () => {
    ...
    setError("");
    try {
      const result = await fetchCatalogListWithCounts(adminApi.listDoseRules, {
        tab,
  ...
  async function openRule(ruleId) {
    ...
    try {
      const rule = await adminApi.getDoseRule(ruleId);
  ...
  async function handleAction(action, ruleId) {
    ...
    try {
      ...
      const result = await adminApi.updateDoseRuleStatus(ruleId, statusByAction[action]);
      setToast(result.message);
      await loadRules();
      if (selectedId === ruleId) {
        const updated = await adminApi.getDoseRule(ruleId);
  ...
  async function handleBulkApprove() {
    ...
    try {
      ...
      const result = await adminApi.bulkApproveDoseRules(payload);
      setToast(result.message);
      clearSelection();
      await loadRules();
    } catch (err) {
    ...
    <div className="admin-page dose-rules-page">
      <header className="admin-page-header">
        <div>
          ...
          <p>
            Review structured dose rules from the ingestion pipeline. Approved rules drive the CDSS dose
      ...
      {!canRead && (
        <div className="admin-banner warning" role="status">
          Sign in with a <strong>clinical_lead</strong> or <strong>admin</strong> account to review counts and approve dose rules.
        </div>
      )}
      ...
      <StatusCountCards
        ...
        hints={{
          ...
          approved: "Active in dose calculator",
        ...
        <section className="admin-table-panel">
          {loading ? (
            ...
            <div className="admin-empty" role="status">
              <h2>No dose rules in this view</h2>
              <p>Run structured dose extraction in the ingestion pipeline, then sync to Postgres.</p>
            </div>
          ) : (
                  ...
                  <tr className={selectedId === rule.id ? "selected" : ""} key={rule.id}>
                    {tab === "draft" && (
                      <td>
                        {rule.status === "draft" ? (
                          <input
                            aria-label={`Select ${rule.dose_rule_id}`}
