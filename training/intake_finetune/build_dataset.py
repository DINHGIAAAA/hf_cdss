...
def build_dataset(
    *,
    ...
    records: list[dict] = []
    stats = {"n2c2": 0, "mimic_demo": 0, "audit": 0}
    ...
    if audit_jsonl:
        audit_records = convert_audit_jsonl(audit_jsonl)
        records.extend(audit_records)
        stats["audit"] = len(audit_records)
...
def main() -> None:
    ...
    parser.add_argument("--audit-jsonl", type=Path, help="Path to exported cdss_audit_events JSONL")
    ...
    if not args.n2c2_dir and not args.mimic_hosp_dir and not args.audit_jsonl:
        parser.error("Provide at least one of --n2c2-dir, --mimic-hosp-dir, or --audit-jsonl")
