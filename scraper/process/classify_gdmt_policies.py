...
def gdmt_policy_tier(policy: dict[str, Any]) -> str:
    ...
    has_reasoning = bool(guidance["reasoning_base"])
    has_actions = bool(guidance["actions"])

    if not has_reasoning and not has_actions:
...
def classify_gdmt_policies(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        annotate(policy, gdmt_policy_tier(policy))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Classify structured GDMT policies."
    )
    parser.add_argument(
        "--input",
        default="artifacts/gdmt_policies/gdmt_policies.jsonl",
        type=Path,
    )
    parser.add_argument(
        "--output",
        default="artifacts/gdmt_policies/gdmt_policies_classified.jsonl",
        type=Path,
    )
    ...
    print(
        f"Wrote {len(classified)} classified GDMT policies to {args.output}"
    )
