...
DEFAULT_MANIFEST = ROOT / "artifacts" / "manifests" / "source_download_manifest.json"
...
def main() -> None:
    ...
    for source in registry.get("sources", []):
        ...
        if args.storage == "s3" and args.use_existing and object_exists(client, args.s3_bucket, key):
            ...
            continue
        if args.storage == "local" and args.use_existing and target.exists():
            rows.append(manifest_row(source, target, "existing"))
            continue
        try:
            if args.storage == "s3":
                payload = download_bytes(source["url"], args.timeout)
                client.put_object(Bucket=args.s3_bucket, Key=key, Body=payload)
                rows.append(
                    manifest_row(
                        source,
                        target,
                ...
                download_url(source["url"], target, args.timeout)
                rows.append(manifest_row(source, target, "downloaded"))
